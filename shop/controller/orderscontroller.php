...
class OrdersController extends BaseController {
	...
	{
		$user= \app\dao\UserInfoDao::getSlaveInstance()->findAll(
			array('level'=>10)
		);
	
		$response->tableCon=$user;
		$this->layoutSmarty();
	}
	 ...
	 * @param unknown_type $response
	 */
	public function userconfirm($request,$response)
	{
		$status = $request->status;
		$user_id= $request->id;
		if($status == 'pass')
		{
			$usr=\app\dao\AdminDao::getSlaveInstance()->find($user_id);
			if(!$usr)
			{
				\app\dao\UserInfoDao::getMasterInstance()->edit($user_id,
					array('level'=>11)
				);
				$user=\app\dao\UserDao::getMasterInstance()->find($user_id);
				\app\dao\AdminDao::getMasterInstance()->add(
	...
	public function index($request,$response)
	{
        ...
        $orderDao = \app\dao\UserEventDao::getSlaveInstance();
		$sql="select count(*) as no from ym_user_event where status <5";
        $ret= $orderDao->getPdo()->getRow($sql);
        $total=$ret['no'];
		...
		$sql = "select * from ym_user_event where status < 5 order by utime desc limit ".$limit;
		$ret = $orderDao->getPdo()->getRows($sql);
