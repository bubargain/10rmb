...
class GoodsController extends BaseController {
    ...
    public function index($request,$response){
		...
		$sql="select count(*) as no from ym_goods where `status` =1";
		...
		$sql = "select * from ym_goods where status =1 order by ctime desc limit ".$limit;
   ...
   public function verify($request,$response){
   		...
   		\app\dao\GoodsDao::getMasterInstance()->edit($goods_id,array(
   			'status' => $status
   		));
