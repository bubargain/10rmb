...
class OrderController extends BaseController {
	...
	public function status($request,$response){
		try{
			...
			if($trade_status['status'] != $status || ($status != 0 && $status!=1) )
				self::renderjson(array('ret'=>'already update'));
	...
	//返利到用户账户
	private function fanli($user_id,$trade_id,$unit="usd")
	{
	    ...
	    \app\dao\UserCurrencyDao::getMasterInstance()->add(array(
			...
			'user_id' => $user_id,
			'ctime' => time(),
	    	'formeramount' =>$user['usd']
		));
		
		
		$newAmount = (float)$user['usd'] + $fanliAmount;
		
		\app\dao\UserInfoDao::getMasterInstance()->edit($user_id,array('usd'=>$newAmount));
	}
