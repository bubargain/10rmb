...
class OrderSrv extends BaseSrv {
    ...
    //创建充值订单
    public function quickSubmit($post)
    {
    	$user = self::checkBuyer($post['user_id']); 
    	$_time = time();
    	$order = array(
        ...
        $id= \app\dao\OrderDao::getMasterInstance()->add($order);
    ...
    private function checkBuyer($user_id) {
        $user = UserInfoDao::getSlaveInstance()->find(array('user_id'=>$user_id));
